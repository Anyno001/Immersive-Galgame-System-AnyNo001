...
export function normalizeSceneTime(value) {
    return TIME_VARIANTS.get(String(value || '').trim().toLowerCase()) || '';
}

export function resolveSceneTimeAsset(baseUrl, time) {
    const source = String(baseUrl || '').trim();
    const variant = normalizeSceneTime(time);
    if (!source || !variant) return source;
    return source.replace(
        /(scene-demo-clean)(?:-(?:dawn|day|dusk|night|minight))?(\.[a-z0-9]+)([?#].*)?$/i,
        `$1-${variant}$2$3`,
    );
}
