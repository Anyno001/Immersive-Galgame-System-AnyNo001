...
- 西欧古典皮肤使用内嵌三片背景：高度 `184px`、左右端各 `110px` 保形、中段横向伸缩，常规最小目标宽度 `280px`；正文继续在现有安全区和滚动节点内排版。
- 西欧古典姓名牌复用 `#igs-speaker` 及其既有显隐条件；按原作者合成坐标相对主框定位为 `left:35px; top:-22px`，高度 `68px`、两端各 `65px` 保形，常规宽度 `374px`，窄框只压缩中段。无姓名时不得出现空牌，切回默认必须恢复原姓名与分隔线布局。
