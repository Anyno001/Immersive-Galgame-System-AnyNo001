...
- 内嵌模式的收纳与关闭按钮固定悬浮在阅读器右上角，保留 32px 透明点击区但全部工具 SVG 可见尺寸固定为 11px、默认低透明度；不得使用工具条底色、描边、阴影或毛玻璃，也不得贴靠对话框或继承普通模式的顶部固定布局。
...
- 内嵌对话框必须完整约束在阅读器容器内：正文区域自行滚动，角色名和分割线不得收缩或被裁出可视范围。
