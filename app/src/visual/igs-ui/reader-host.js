...
export function createIgsReaderHost(options = {}) {
    ...
    function renderSettingsBody(tab, draft, asyncState) {
        ...
        const readerValues = {
            ...
            dialogHeightField: field('readerSettings.dialogHeight', '对话框高度', selectInput('readerSettings.dialogHeight', reader.dialogHeight === null ? 'null' : reader.dialogHeight, dialogHeightItems, classicDialog), classicDialog ? `当前风格使用固定 ${CLASSIC_DIALOG_HEIGHT}px；原设置值会保留。` : '最低 60px，正文过长时在框内滚动。'),
