...
export function normalizeSettingsValue(path, value) {
    ...
    if (path.startsWith('readerSettings.')) {
        ...
        if (/^readerSettings\.statusHud\.enabled$/.test(path) || /^readerSettings\.statusHud\.showEmotion$/.test(path) || /^readerSettings\.statusHud\.showLocation$/.test(path) || /^readerSettings\.statusHud\.showLocationDetails$/.test(path)) {
