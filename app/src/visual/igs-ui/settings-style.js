...
.igs-settings-shell{width:var(--igs-settings-width);height:var(--igs-settings-height);max-height:none;background:var(--igs-settings-panel);border:0;border-radius:2px;box-shadow:none;display:flex;flex-direction:column;overflow:hidden;-webkit-backdrop-filter:none;backdrop-filter:none}
...
.igs-settings-close,.igs-settings-action{border:0;background:var(--igs-settings-field);color:var(--igs-settings-ink-2);border-radius:2px;height:34px;padding:0 12px;font:inherit;font-size:12px;letter-spacing:.04em;cursor:pointer;transition:background-color .14s ease,color .14s ease}
...
.igs-settings-tabs{display:flex;gap:2px;padding:8px 12px;overflow-x:auto;flex-shrink:0;background:var(--igs-settings-surface);border-bottom:1px solid var(--igs-settings-line)}
.igs-settings-tab{border:0;background:transparent;color:var(--igs-settings-ink-3);padding:7px 10px;border-radius:2px;font:inherit;font-size:12px;white-space:nowrap;cursor:pointer;transition:background-color .14s ease,color .14s ease}
...
.igs-settings-tab.is-active{background:var(--igs-settings-highlight);color:var(--igs-settings-ink);box-shadow:none}
.igs-scene-subtabs{display:flex;gap:2px;margin:8px 0;padding:3px;background:var(--igs-settings-field);border-radius:2px}
.igs-scene-subtab{flex:1;border:0;background:transparent;color:var(--igs-settings-ink-3);padding:7px 10px;border-radius:1px;font:inherit;font-size:12px;white-space:nowrap;cursor:pointer}
...
.igs-settings-field input,.igs-settings-field select,.igs-settings-field textarea{width:100%;box-sizing:border-box;border:0;border-bottom:1px solid transparent;background:var(--igs-settings-field);color:var(--igs-settings-ink);border-radius:2px;padding:8px 10px;font:inherit;font-size:13px;line-height:1.5;outline:none;transition:background-color .14s ease,border-color .14s ease}
.igs-settings-field option{background:var(--igs-settings-panel);color:var(--igs-settings-ink)}
.igs-settings-field input[type="color"]{width:42px;height:36px;padding:3px;border-radius:2px;cursor:pointer}
.igs-settings-field input[type="color"]::-webkit-color-swatch-wrapper{padding:0}
.igs-settings-field input[type="color"]::-webkit-color-swatch{border:0;border-radius:1px}
.igs-scene-preset-select{box-sizing:border-box;border:0;border-bottom:1px solid transparent;background:var(--igs-settings-field);color:var(--igs-settings-ink);border-radius:2px;padding:8px 10px;font:inherit;font-size:13px;outline:none;height:38px}
...
.igs-settings-secret-toggle{height:36px;min-width:52px;border:0;background:var(--igs-settings-field);color:var(--igs-settings-ink-3);border-radius:2px;font:inherit;font-size:12px;cursor:pointer}
...
.igs-segmented{height:40px;display:grid;grid-template-columns:repeat(var(--igs-segment-count,3),minmax(0,1fr));align-items:center;gap:2px;padding:4px;box-sizing:border-box;border:0;background:var(--igs-settings-field);border-radius:2px;position:relative;overflow:hidden}
.igs-segmented-indicator{display:none}
.igs-segmented-btn{height:32px;min-width:0;position:relative;z-index:1;border:0;border-radius:1px;background:transparent;color:var(--igs-settings-ink-3);font:inherit;font-size:12px;line-height:18px;font-weight:500;letter-spacing:0;white-space:nowrap;cursor:pointer;padding:0 6px;display:inline-flex;align-items:center;justify-content:center;gap:5px;overflow:hidden;transition:background-color .14s ease,color .14s ease}
...
.igs-switch{height:38px;display:flex;align-items:center;gap:10px;border:0;background:var(--igs-settings-field);color:var(--igs-settings-ink-2);border-radius:2px;padding:0 12px;cursor:pointer;text-align:left;font:inherit;font-size:13px;transition:background-color .14s ease,color .14s ease}
.igs-switch:hover,.igs-switch:focus-visible{background:var(--igs-settings-highlight);color:var(--igs-settings-ink);outline:none}
.igs-switch i{width:30px;height:18px;border-radius:2px;background:var(--igs-settings-line-strong);position:relative;flex-shrink:0}
.igs-switch i:after{content:"";position:absolute;width:14px;height:14px;top:2px;left:2px;border-radius:1px;background:var(--igs-settings-ink)}
...
.igs-source-filter{grid-column:1/-1;padding:12px;border:0;border-radius:2px;background:var(--igs-settings-surface);display:flex;flex-direction:column;gap:12px}
.igs-source-filter-title{font-size:12px;line-height:18px;font-weight:500;letter-spacing:.06em;color:var(--igs-settings-ink)}
.igs-source-filter-note{font-size:11px;line-height:16px;font-weight:400;color:var(--igs-settings-ink-4)}
...
.igs-settings-preview{white-space:pre-wrap;max-height:220px;overflow:auto;font-family:ui-monospace,Consolas,monospace;font-size:12px;line-height:1.55;border:0;border-radius:2px;background:var(--igs-settings-paper);padding:12px;color:var(--igs-settings-ink-2)}
.igs-btn-mgr-list{display:flex;flex-direction:column;gap:0;border:0;border-radius:2px;background:var(--igs-settings-paper);padding:4px 6px;overflow:hidden}
.igs-btn-mgr-row{display:flex;align-items:center;gap:8px;height:36px;padding:0 8px;border-radius:0;border-bottom:1px solid var(--igs-settings-line);transition:background-color .14s ease}
...
.igs-btn-mgr-icon{border:0;background:transparent;color:var(--igs-settings-ink-4);cursor:pointer;padding:4px;border-radius:1px;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:color .14s ease,background-color .14s ease}
...
.igs-scene-url-input{flex:1;min-width:0;height:28px;border:0;border-bottom:1px solid transparent;background:var(--igs-settings-field);color:var(--igs-settings-ink);border-radius:2px;padding:0 8px;font:inherit;font-size:11px;outline:none}
.igs-scene-url-input:focus{border-bottom-color:var(--igs-settings-line-strong);background:var(--igs-settings-highlight)}
.igs-scene-char-group{margin-bottom:8px;border:0;border-bottom:1px solid var(--igs-settings-line);border-radius:0;padding:4px;background:transparent}
...
.igs-mood-word-tag{display:inline-flex;align-items:center;gap:4px;font-size:12px;padding:2px 6px;border-radius:2px;background:var(--igs-settings-highlight);border:0;color:var(--igs-settings-ink-2)}
...
.igs-sprite-slot{border-bottom:1px solid var(--igs-settings-line)}
...
.igs-sprite-thumb{width:72px;height:72px;flex-shrink:0;object-fit:contain;border-radius:2px;background:var(--igs-settings-paper);border:0;cursor:zoom-in}
...
.igs-sprite-preview-img{max-width:100%;max-height:100%;object-fit:contain;border-radius:2px;box-shadow:none}
@media (max-width:640px){#igs-unified-settings{--igs-settings-width:min(760px,calc(var(--igs-settings-vw) - 24px));--igs-settings-height:min(760px,calc(var(--igs-settings-vh) - 24px));padding:max(8px,env(safe-area-inset-top)) max(8px,env(safe-area-inset-right)) max(8px,env(safe-area-inset-bottom)) max(8px,env(safe-area-inset-left))}.igs-settings-shell{width:var(--igs-settings-width);height:var(--igs-settings-height);border-radius:2px}.igs-settings-grid,.igs-settings-api-group,.igs-source-filter-grid{grid-template-columns:1fr}.igs-segmented{height:40px}.igs-segmented-btn{font-size:11px;padding:0 4px;gap:4px}}
`.trim();
