...
- 发布：当前发布线为 `v0.24.0`；版本标签留待真实酒馆复验后处理。
- 当前项目版本 `v0.24.0`：新增默认关闭的打字机演出与快/中/慢三档速度；阅读器设置重分为「对话框 / 画面 / 演出 / 选项 / 顶部UI」，原持久化字段路径保持不变；NSFW 隐藏人物立绘时，对白与心理页继续显示已开启的地点栏，但不显示头像和情绪。
