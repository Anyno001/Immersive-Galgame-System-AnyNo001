...
- 发布：本轮 `v0.23.63` 推送 `origin/main`，版本标签留待真实酒馆复验后处理。
- 当前项目版本 `v0.23.63`：状态栏小、中、大三档在原响应式尺寸基础上整体缩放至 `80%`，包括头像、HUD 条与折叠控件；恢复可选的「跟随对话框」背景，背景色透明度取对话框的 `50%` 并缩小圆角，「无背景」选项保持透明。
