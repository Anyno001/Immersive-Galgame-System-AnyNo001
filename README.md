...
- 发布：本轮 `v0.23.65` 推送 `origin/main`，版本标签留待真实酒馆复验后处理。
- 当前项目版本 `v0.23.65`：状态栏末尾数值改为固定等宽列左对齐，使各 HUD 条右端一致；展开态移除减号，点击整块状态栏即可收起，折叠态保留加号展开；「状态栏背景」由下拉菜单改为与 HUD 条配色一致的「无背景 / 跟随对话框」分段可选按钮，原有值与持久化语义不变。
