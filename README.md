...
- 未发布：本轮改动已推送 `origin/main`，版本标签留待真实酒馆复验后处理。
- 当前项目版本 `v0.23.58`：新增默认关闭的左上角角色状态栏（头像、原始情绪胶囊、最多四条数值 HUD），并将默认皮肤普通、浮窗、手机浮窗、楼层内嵌模式的顶部内边距压缩至约原值的 2/3；状态栏与默认皮肤改动已包含在源码和构建产物中。
