...
- 发布：本轮 `v0.23.59` 已推送 `origin/main`，版本标签留待真实酒馆复验后处理。
- 当前项目版本 `v0.23.59`：修复状态栏表格按钮未接入真实点击链，避免只有头像与情绪而无数据库 HUD 条；读取表格改为独立按钮组，说明另起一行且只高亮已选表；恢复角色卡「状态栏头像」行；场景素材多层列表补齐窄容器收缩边界，避免横向溢出。
