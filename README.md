...
- 当前推送候选：`v0.27.2`；地图读取「场景地点表」并以「重要角色表.所在地点」关联 NPC，人物关系页优先读取重要角色档案及「人际关系」；保留旧地图和旧人物关系表兼容，不把「在场状态」当位置，也不把「关系网络表」组织资料当人物。
- 验证边界：模板导入由用户实测；`table/` 保留为已忽略的本地文件，默认单测不依赖该目录，因此仓库内不提供此模板的 DDL 回归校验。地图及关系页面在有真实数据行时仍需酒馆复验；本地测试不能替代真实页面验收。真机通过前不打 tag。
