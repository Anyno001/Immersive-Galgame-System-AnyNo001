...
- 当前项目版本 `v0.23.57`：默认皮肤所有形态（普通、浮窗、手机浮窗、楼层内嵌）的名字与对话框上沿之间的顶部内边距统一压缩至约原值的 2/3；姓名样式及左右、底部内边距保持原值。本轮按需求跳过完整门禁。
- `v0.23.57` 未闭环项：`app/dist/igs.bundle.js` 与 `app/dist/manifest.json` 本轮未随源码提交。构建环境同时存在并行未提交改动，重跑 `package:loader` 会把它们一并打进产物，无法在产物层干净剔除；需在并行改动落定后重跑 `package:loader` 并单独提交产物。源码 `npm.cmd run package:loader` 已验证 `gate:build ok`、`loader:build ok`、`loader:debug ok`。
